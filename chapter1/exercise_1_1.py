...
def train(epochs=10000, print_every_n=500, ep1=0.1, ep2=0.1):
    ...
    player1.save_policy()
    player2.save_policy()
    return player1_win / epochs, player2_win / epochs
...
def ex11():
    ...
    if training:
        ...
        np.save("../data/exercies_1_1_p1_winrate_train.npy", p1_win_rate_grid)
        np.save("../data/exercies_1_1_p2_winrate_train.npy", p2_win_rate_grid)
    ...
    for i in range(len(ep1_list)):
        for j in range(len(ep2_list)):
            text = ax3.text(j, i, p1_win_rate_grid[i, j],
    ...
    im = ax4.imshow(p1_win_rate_grid)
